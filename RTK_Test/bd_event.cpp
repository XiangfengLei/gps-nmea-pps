...
static const std::map<std::string, int> bestpos_type_map_ = {
    {"NONE", 0},             // 无解
    ...
    {"PPP_CONVERAGE", 68},   // PPP 状态收敛中
    {"PPP", 69},             // PPP 定位
};

BDUnicore::BDUnicore(const bool &debug) : debug_(debug)
{
}
...
bool BDUnicore::Parser(const std::string &unicore_msg, UnicoreData &data)
{
    if (debug_)
    {
        printf("RTk unicore: %s", unicore_msg.c_str());
    }

    bool ret = false;
    std::vector<std::string> unicore_splits;
    bool is_unicore = CheckCRC32(unicore_msg);
    if (!is_unicore)
    {
        printf("Unicore checksum fail: %s!\r\n", unicore_msg.c_str());
        return false;
    }

    unicore_splits.clear();
    unicore_splits = SpliteString(unicore_msg, ";");
    if (unicore_splits.size() != 2)
    {
        return false;
    }

    UnicoreType type = UnicoreType::UNKNOWN;
    if (unicore_splits[0].find("PVTSLNA") != std::string::npos)
    {
        type = UnicoreType::PVTSLN;
    }

    switch (type)
    {
    case UnicoreType::PVTSLN:
        ret = ParserPVTSLN(unicore_splits, data);
        break;
...
bool BDUnicore::CheckCRC32(const std::string &unicore_msg)
{
    size_t len = unicore_msg.length();
    if (len < 8)
    {
        return false;
    }

    size_t pos_asterisk = unicore_msg.find("*");
    if (pos_asterisk == std::string::npos)
    {
        return false;
    }

    char unicore_sum[9] = {0};
    memcpy(unicore_sum, unicore_msg.data() + pos_asterisk + 1, 8);
    unsigned long sum_value = strtoul(unicore_sum, nullptr, 16); // string to long

    unsigned long result = 0;
    for (size_t i = 1; unicore_msg[i] != '*'; ++i) // # 不参与计算 crc32
    {
        result = aulCrcTable[(result ^ unicore_msg[i]) & 0xff] ^ (result >> 8);
    }

    // RNS_DBUG("Unicore sum_value %08lx result %08lx\r\n", sum_value, result);
    if (sum_value != result)
    {
        printf("Unicore check sum fail, sum %08lx result %08lx\r\n", sum_value, result);
        return false;
    }
...
bool BDUnicore::ParserPVTSLN(const std::vector<std::string> &unicore_splits, UnicoreData &data)
{
    std::vector<std::string> header;
    std::vector<std::string> content;
    header = SpliteString(unicore_splits[0], ",");
    content = SpliteString(unicore_splits[1], ",");

    /* ------------------------- HEADER -----------------------------------*/
    data.pvtsln.time_status = header[3];
    data.pvtsln.week = strtol(header[4].c_str(), nullptr, 10);
    data.pvtsln.week_ms = strtol(header[5].c_str(), nullptr, 10);

    /* ------------------------- CONTENTS -----------------------------------*/
    data.pvtsln.bestpos_type = bestpos_type_map_.at(content[0]);
    data.pvtsln.bestpos_hgt = strtof(content[1].c_str(), nullptr);
    data.pvtsln.bestpos_lat = strtod(content[2].c_str(), nullptr);
    data.pvtsln.bestpos_lon = strtod(content[3].c_str(), nullptr);
    data.pvtsln.bestpos_hgtstd = strtof(content[4].c_str(), nullptr);
    data.pvtsln.bestpos_latstd = strtof(content[5].c_str(), nullptr);
    data.pvtsln.bestpos_lonstd = strtof(content[6].c_str(), nullptr);
    data.pvtsln.bestpos_diffage = strtof(content[7].c_str(), nullptr);
    data.pvtsln.undulation = strtof(content[12].c_str(), nullptr);
    data.pvtsln.bestpos_svs = strtol(content[13].c_str(), nullptr, 10);
    data.pvtsln.bestpos_solnsvs = strtol(content[14].c_str(), nullptr, 10);
    data.pvtsln.pdop = strtof(content[29].c_str(), nullptr);
    data.pvtsln.hdop = strtof(content[30].c_str(), nullptr);
    data.pvtsln.vdop = std::sqrt(std::pow(data.pvtsln.pdop, 2) - std::pow(data.pvtsln.hdop, 2));
    data.pvtsln.enable = true;
    if (debug_)
    {
        printf("RTK PVTSLN: %s,%d,%ld,%d,%f,%f,%f,%f,%f,%f,%f,%f,%d,%d,%f,%f,%f\r\n",
               data.pvtsln.time_status.c_str(), data.pvtsln.week, data.pvtsln.week_ms, data.pvtsln.bestpos_type,
               data.pvtsln.bestpos_hgt, data.pvtsln.bestpos_lat, data.pvtsln.bestpos_lon,
               data.pvtsln.bestpos_hgtstd, data.pvtsln.bestpos_latstd, data.pvtsln.bestpos_lonstd,
               data.pvtsln.bestpos_diffage, data.pvtsln.undulation,
               data.pvtsln.bestpos_svs, data.pvtsln.bestpos_solnsvs,
               data.pvtsln.pdop, data.pvtsln.hdop, data.pvtsln.vdop);
    }
    return true;
}
